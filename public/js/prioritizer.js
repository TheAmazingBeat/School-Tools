...
function homeworkClickHandler(e) {
	...
	if (e.target != prevClicked && parentOne[0] != parentTwo[0]) {
		...
		$(parentTwo).find('.name-input').find('.fa-trash-can').removeClass('visible');
	}
	...
	if (!$(e.target).closest('.homework-item').length && homeworkClicked) {
		...
		$(parentOne).find('.name-input').find('.fa-trash-can').removeClass('visible');
...
function homeworkHoverHandler(e) {
	...
	const parentOne =
		// Hover over input
		$(e.target).hasClass('hw-check') || $(e.target).hasClass('hw-name') || $(e.target).hasClass('fa-trash-can')
			? $(e.target).parents('.homework-item')[0]
			: e.target == $('.homework-list')[0]
			? $(e.target).find('.homework-item')[0]
			: e.target;
	// else if (e.target == $(e.target)[0]) parentOne = e.target;
	const parentTwo = prevHovered;
	...
	// if (e.type == 'mouseenter' && e.target != prevHovered && parentOne[0] != parentTwo[0]) {
	// }
	...
	prevHovered = parentOne;

	// if (e.type == 'mouseenter') {
	// 	// if ($(e.target).hasClass('.hw-check')) parent = $(e.target).parents('.homework-item');
	// 	// else if (e.target == $(e.target)[0]) parent = e.target;
	// 	// console.log(parent);

	// 	// $(parent).find('.name-input').find('.hw-check').addClass('visible');
	// 	// $(parent).find('.name-input').find('svg').addClass('visible');

	// } else if (e.type == 'mouseleave') {
	//   $(prevHovered).find('.name-input').find('.hw-check').removeClass('visible');
	// 	$(prevHovered).find('.name-input').find('.fa-trash-can').removeClass('visible');
	// }
	// prevHovered = parent;
}
...
function addHW(isStored, hwObject) {
	...
	$(homework.element).focusin((e) => {
		...
		$(e.currentTarget).find('.name-input').find('.hw-check').addClass('visible');
