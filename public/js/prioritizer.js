...
function addHW(isStored, hwObject) {
  ...
  $(homework.element).blur((e) => {
		$(e.currentTarget).find('.options').removeClass('visible');
		$(e.currentTarget).find('.name-input').find('.hw-check').removeClass('visible');
		$(e.currentTarget).find('.name-input').find('svg').removeClass('visible');
  });
